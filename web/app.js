...
(() => {
  ...
  const SCOPE_URL = CFG.scopeUrl || "http://192.168.10.32/Instrument/novnc/vnc_auto.php";
  ...
  function buildBar() {
    ...
    for (const p of PANELS) {
      ...
      bar.appendChild(el);
      if (p.id === "lcr") {            // placeholder (no data yet): top-left "LCR", value "—",
        el.querySelector(".mode").textContent = "LCR";   // 2nd row hidden, grey/transparent like an OFF device
  ...
  function renderState(state) {
    ...
    if (d.dmm) {
      ...
      el.querySelector(".val.v").innerHTML = valHtml(fmtDmm(s.value, s.unit || "", s.mode, s.range));
      // Diode + |value| < 0.05 V -> "Shorted" (red) in the second row
      const isShorted = s.mode === "Diode" && s.value != null && isFinite(s.value) && Math.abs(s.value) < 0.05;
    ...
    for (const id of ["bb3a", "bb3b", "kel"]) {
      ...
      el.querySelector(".val.v").innerHTML = off ? offHtml : valHtml({ sign: "", num: fmt3(s.voltage), unit: "V" });
      el.querySelector(".val.a").innerHTML = off ? offHtml : valHtml({ sign: "", num: fmt3(s.current), unit: "A" });
      if (id === "bb3b") {
  ...
  // =====================================================================
  let curStream = null;
  // Auto-select camera — no selection UI. Prefers the microscope
  // (label heuristic), then the last-used, then the first camera.
  // Never use the HT-301 thermal camera as viewfinder (browser only gets raw YUYV;
  // it runs via the labcam-thermal service). Microscope selected primarily by USB-ID
  // (Chromium appends "(vid:pid)" to the label): eba4:7588 = "USB3.0 HD Video Capture".
  const IS_IR = (d) => /t3-?317|thermal|infiray|ht-?301|\bhti\b/i.test(d.label || "");
  function pickMicro(devs) {
    const MICRO_USBID = (CFG.microUsbId || "eba4:7588").toLowerCase();
    const cams = devs.filter(d => d.kind === "videoinput" && !IS_IR(d));
    const saved = LS.getItem("cam_id");
    const byId = cams.find(d => (d.label || "").toLowerCase().includes(MICRO_USBID));
    const byName = cams.find(d => /usb.?3|hd video|capture|mikroskop|microscope/i.test(d.label || ""));
    const savedOk = saved && cams.some(d => d.deviceId === saved);
    return (byId && byId.deviceId) || (byName && byName.deviceId)
        || (savedOk ? saved : null) || (cams[0] && cams[0].deviceId);
  }
  const enumCams = async () => {
    try { return await navigator.mediaDevices.enumerateDevices(); } catch (e) { return []; }
  };
  async function initCamera() {
    // 1) Enumerate WITHOUT probing. On reload permission is already granted ->
    //    labels are available -> open microscope directly by USB-ID (no double-open, never video2).
    let devs = await enumCams();
    if (devs.some(d => d.kind === "videoinput" && d.label)) {
      const pick = pickMicro(devs);
      if (pick) { await startCam(pick); return; }
    }
    // 2) First visit (no permission yet -> no labels): probe once, then pick deliberately.
    let probe = null;
  ...
  function cropBase64(nrm) {
    ...
    c.getContext("2d").drawImage(video, cx, cy, cw, ch, 0, 0, ow, oh);
    return c.toDataURL("image/jpeg", 0.9).split(",")[1];
  }
  async function identify() {
    if (!compOn || !roi || !video.videoWidth) return;
